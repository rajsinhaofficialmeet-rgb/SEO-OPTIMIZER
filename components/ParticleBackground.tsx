...
export const ParticleBackground: React.FC = () => {
    ...
    const particleColorRef = useRef('rgba(245, 158, 11, 0.7)');
    const lineColorRef = useRef('rgba(245, 158, 11, 0.2)');

    const updateColors = () => {
        const isDarkMode = document.documentElement.classList.contains('dark');
        particleColorRef.current = isDarkMode ? 'rgba(251, 191, 36, 0.7)' : 'rgba(245, 158, 11, 0.7)'; // amber-400 / amber-500
        lineColorRef.current = isDarkMode ? 'rgba(251, 191, 36, 0.2)' : 'rgba(245, 158, 11, 0.2)';
    };
            ...
            for (let a = 0; a < particles.length; a++) {
                for (let b = a; b < particles.length; b++) {
                    ...
                    if (distance < (canvas.width / 7) * (canvas.height / 7)) {
                        ...
                        ctx.strokeStyle = lineColorRef.current.replace(/[\d\.]+\)/, `${opacityValue})`);
                        ctx.lineWidth = 1;
                        ctx.beginPath();
    ...
    return <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full -z-20 block pointer-events-none" />;
};
